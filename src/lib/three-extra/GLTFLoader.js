...
module.exports = ({THREE}) => {
	...
	GLTFLoader.prototype = {
			...
			loader.load( url, function ( data ) {
				...
				try {
					...
					if ( onError !== undefined ) {
						...
						onError( e.constructor === Error ? e : new Error( 'THREEGLTFLoader: Unable to parse model.' ) );
		...
		parse: function ( data, path, onLoad, onError ) {
			...
			if ( json.asset === undefined || json.asset.version[ 0 ] < 2 ) {
				...
				onError( new Error( 'THREEGLTFLoader: Unsupported asset. glTF versions >=2.0 are supported.' ) );
				return;
		...
		for ( var lightId in lights ) {
			...
			if ( lightNode ) {
				...
				if ( light.fallOffExponent !== undefined ) {
					...
					console.warn( 'THREEGLTFLoader:: light.fallOffExponent not currently supported.' );
	...
	function GLTFBinaryExtension( data ) {
		...
		if ( this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC ) {
			...
			throw new Error( 'THREEGLTFLoader: Unsupported glTF-Binary header.' );

		} else if ( this.header.version < 2.0 ) {

			throw new Error( 'THREEGLTFLoader: Legacy binary file detected. Use GLTFLoader instead.' );
		...
		if ( this.content === null ) {
			...
			throw new Error( 'THREEGLTFLoader: JSON content not found.' );
	...
	GLTFParser.prototype.loadBuffer = function ( bufferIndex ) {
		...
		if ( bufferDef.type && bufferDef.type !== 'arraybuffer' ) {
			...
			throw new Error( 'THREEGLTFLoader: ' + bufferDef.type + ' buffer type is not supported.' );
		...
		return new Promise( function ( resolve, reject ) {
			...
			loader.load( resolveURL( bufferDef.uri, options.path ), resolve, undefined, function () {
				...
				reject( new Error( 'THREEGLTFLoader: Failed to load buffer "' + bufferDef.uri + '".' ) );
	...
	GLTFParser.prototype.loadTexture = function ( textureIndex ) {
		...
		return Promise.resolve( sourceURI ).then( function ( sourceURI ) {
			...
			if ( textureDef.internalFormat !== undefined && texture.format !== WEBGL_TEXTURE_FORMATS[ textureDef.internalFormat ] ) {
				...
				console.warn( 'THREEGLTFLoader: Three.js does not support texture internalFormat which is different from texture format. ' +
				...
				return scope.loadGeometries( primitives ).then( function ( geometries ) {
					...
					for ( var i = 0; i < primitives.length; i ++ ) {
						...
						if ( material.aoMap
							...
							console.log( 'THREEGLTFLoader: Duplicating UVs to support aoMap.' );
						...
						if ( primitive.mode === WEBGL_CONSTANTS.TRIANGLES || primitive.mode === undefined ) {
							...
							throw new Error( 'THREEGLTFLoader: Primitive mode unsupported: ', primitive.mode );
	...
	GLTFParser.prototype.loadCamera = function ( cameraIndex ) {
		...
		if ( ! params ) {
			...
			console.warn( 'THREEGLTFLoader: Missing camera parameters.' );
			return;
					...
					if ( sampler ) {
						...
						if ( node ) {
							...
							if ( sampler.interpolation === 'CATMULLROMSPLINE' ) {
								...
								console.warn( 'THREEGLTFLoader: CATMULLROMSPLINE interpolation is not supported. Using CUBICSPLINE instead.' );
						...
						for ( var i = 0; i < meshes.length; i ++ ) {
							...
							for ( var j = 0, l = skinEntry.joints.length; j < l; j ++ ) {
								...
								if ( jointNode ) {
									...
									console.warn( 'THREEGLTFLoader: Joint "%s" could not be found.', jointId );
